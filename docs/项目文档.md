...
| 图备注/纹理属性到代码变量命名 | 已实现受限规则 | `VertexAnimation` 有 1 个实际源码变量被重命名；粒子加色预设的纹理采样变量被命名 |
...
| 公开仓库 Git 安装与样本扩展 | 已验证 2022.3.62f3 的 20 份上游样本 | 全新工程安装公开 UPM 包；10 个预设、10 个示例的原/导出 Shader 均无导入错误，见[验证记录](./验证记录.md) |
| 画面/材质/平台等价 | 初步验证一份上游示例 | `VertexAnimation` 在 Metal 上两组参数逐像素一致；用户材质、更多场景和平台未验证，不输出 `Full` |
| 通用逻辑链及全部节点 | 未实现 | 未建立逐节点兼容矩阵 |

## 7. 当前需要的开发输入

继续推进 P0 需要 **3–6 个用户实际使用的 Shader Forge `.shader`**，最好包含对应 `.mat`、纹理及一份能看到效果的场景。当前已有公开 Unity 包和独立核心测试：上游 20 份预设/示例的导出 Shader 在 Unity 2022.3.62f3 中导入成功且无编译错误；仍缺用户真实样本、材质/画面对照和完整逻辑链重构。
